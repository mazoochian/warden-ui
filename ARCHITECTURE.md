...
| Safe behavioral tunables, genuinely live | `WARDEN_RETENTION_MESSAGES`, `WARDEN_DIGEST_INTERVAL_SECONDS`, `WARDEN_LLM_OWNER_ONLY`, `WARDEN_LLM_SHOW_THINKING`, `WARDEN_LLM_STREAMING`, `WARDEN_LLM_MAX_TOKENS`, `WARDEN_LLM_HISTORY_MESSAGES`, `WARDEN_LLM_SKIP_TRIVIAL_MESSAGES` | **Implemented (2026-07-28).** In `dynamic_config` (env var is the fallback default). Genuinely live: each of these was confirmed to already be read fresh per-message or per-scheduled-tick, not cached at startup. The six LLM-turn ones are fetched together in one `dynamic_config.listAll` call (`main.zig`'s `resolveLlmDynamicSettings`) rather than six separate round trips — this codebase has already hit real Postgres pool exhaustion under load once, so minimizing per-message DB calls isn't theoretical here. |
| Safe behavioral tunables, restart-required | pool size, acquire/statement timeouts, `WARDEN_WORKERS_PER_PLATFORM`, confirm/convert/menu timeout seconds | **Correction (2026-07-28):** the original version of this table listed these alongside the genuinely-live tunables above under one "safe behavioral tunables" row, claiming "most of these are already read per-message/per-tick." Checking actual read sites while implementing Phase 3 found that's false for these five specifically — each is read exactly once, at startup, to size/construct a fixed structure (`PgPool`, each connector's `WorkerPool`, `PendingConfirmations`, `PendingConversions`, `menu.Sessions`). Making them genuinely live needs those structures to re-check `dynamic_config` themselves (a real, separate refactor, not "just add a DB read") — out of scope for Phase 3, not wired into `dynamic_config` at all for now. Stay env-only, same as before this table existed. |
| Provider selection | `WARDEN_LLM_PROVIDER` | **Implemented (2026-07-28).** In `dynamic_config` (`.string` kind — see `store/dynamic_config.zig`'s `known_keys`), genuinely hot-swappable: `main.zig` now constructs *both* providers at startup whenever both have credentials (previously `config.zig` only ever loaded the selected one's env vars — a real blocker for hot-swap, fixed as part of this). New `llm/dynamic_provider.zig`'s `DynamicLlmProvider` wraps both behind one `llm.Provider` interface and re-checks `dynamic_config` on every call, falling back to whichever provider is actually configured if the requested one isn't (never leaves the bot with no working provider). `PATCH` rejects a value naming a provider with no credentials configured, rather than accepting it and silently falling back. |
| Model selection | `WARDEN_ANTHROPIC_MODEL`, `WARDEN_OPENAI_MODEL`, `WARDEN_OPENAI_BASE_URL` | **Correction (2026-07-28):** this row originally claimed "swapping models live is safe and useful," implying it already worked. Checking the actual code while implementing provider hot-swap found that's false — the model string is baked into `AnthropicProvider`/`OpenAiCompatProvider` at construction time, exactly the same way the provider *kind* was, and `llm.ChatRequest` has no per-call model field to override it through either. Model selection is just as startup-fixed as provider selection was — **not wired into `dynamic_config`**, stays env-only for now. Making it live would mean either adding a model field to `ChatRequest` or having each provider re-read `dynamic_config` inside its own `chat()`/`chatStream()` — either touches the actual outbound-request-building code in `llm/anthropic.zig`/`llm/openai_compat.zig`, a different (and somewhat riskier) piece of surgery than the provider-kind swap, deliberately not done in the same pass. |
