...
import { clickableRowProps, EmptyState, PageHeader, PlatformBadge, Section, TableScroll, useCommonStyles } from "@/components/ui-kit";
    ...
    <div className={s.page}>
      ...
      {data && (
        <Section title={data.items.length === 1 ? t("groups.countOne") : t("groups.countOther", { count: data.items.length })}>
          ...
          {data.items.length > 0 && (
            <TableScroll label={t("groups.title")} minWidth={460}>
              <Table>
                <TableHeader>
                  <TableRow>
